...
[CreateAssetMenu(menuName = "Card Effects/Attack Dispel")]
public class AttackDispelEffect : CardEffect
{
    ...
    [Range(0, 100)]
    public int dispelChance = 75; // % tỷ lệ xóa buff (75 = 75%)
    ...
    public override void Execute(PlayerCharacter player, EnemyCharacter enemy)
    {
        ...
        if (enemy == null)
        {
            Debug.LogWarning("AttackDispelEffect: No valid enemy target!");
            return;
        }

        if (enemy.GetCurrentHP() <= 0)
        {
            Debug.LogWarning("AttackDispelEffect: Target enemy is already dead!");
            return;
        }
        ...
        // Dispel buff với tỷ lệ phần trăm
        TryDispelBuff(enemy);
    ...
    private void TryDispelBuff(EnemyCharacter enemy)
    {
        // Kiểm tra enemy có buff không
        if (enemy.GetBuffs().Count == 0)
        {
            return;
        }
